...
//    }
    ...
    void insertByValue(int val, Node *node) {
        auto p = splitByValue(root, val);
        root = merge(merge(p.first, node), p.second);
    }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        switch (mode) {
            case 1:
                ...
                break;
            case 0:
                cout << treapArray.get(treapArray.root, treapArray.getSize(treapArray.root) - t)->val << '\n';
                break;
            case -1:
                treapArray.removeByValue(t);
                break;
        }
